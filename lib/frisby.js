...
function Frisby(msg) {
  ...
  // by default, it should be a single request that does not poll
  this.poll = null;
...
Frisby.prototype._request = function (/* method [uri, data, params] */) {
  ...
  //
  // If polling is enabled, we should attach the evaluator to the outgoing request
  //
  if(this.poll){
    outgoing.retryStrategy = function ( err, response ){
      if(self.poll.pollFormat == 'json'){
        var respJSON = null;
        try{
        ...
        return ! self.poll.evaluator( respJSON ); 
      }else{
        var respBody = response ? response.body : null;
        return ! self.poll.evaluator( err, response, respBody );
      }
      // each return is reversed because the requestretry library asks for a retry condition, and the "pollUntil" language infers a success condition
    };
    outgoing.maxAttempts = this.poll.maxAttempts;
    outgoing.retryDelay = this.poll.delay;
  }else{
    outgoing.retryStrategy = function () { return false; }
  }
  ...
  //
  var runner = params.mock || ( this.poll ? requestretry : request );
...
Frisby.prototype.pollUntil = function ( evaluator, delay, maxAttempts ){
  if(this.poll){ 
    console.error("Polling already requested for this test."); 
  }else if(
    typeof(evaluator) != "undefined" && 
    typeof(delay) != "undefined" && 
    typeof(maxAttempts) != "undefined"
    ){
    this.poll = {
      evaluator : evaluator,
      delay : delay,
    ...
    console.error("Missing arguments for pollUntil");
  }
  return this;
}

Frisby.prototype.pollUntilJSON = function ( evaluator, delay, maxAttempts ){
  if(this.poll){ 
    console.error("Polling already requested for this test."); 
  }else if(
    typeof(evaluator) != "undefined" && 
